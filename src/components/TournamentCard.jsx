  ...
  return (
    <div className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden h-full flex flex-col">
      <div className="p-6 pb-2 flex justify-between items-start gap-3">
        <h3 className="text-lg font-black text-gray-900 leading-tight">{tournament.title}</h3>
        <StatusBadge status={tournament.status} />
      </div>

      {/* 🚀 Navigation (Challenge 5): Hna fin ghadi n-sta3mlo setActiveTab */}
      <div className="flex px-6 mb-4 gap-2 bg-white">
        <button 
          // Hna sta3mlnaha bach l-warning y-7iyd w l-onglet y-tbeddel
          onClick={() => setActiveTab("info")} 
          className={`flex-1 py-2 rounded-full text-xs font-bold transition-all ${
            activeTab === 'info' ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-400'
          }`}
        >
          Info
        </button>
        ...
        <button 
          ...
          className={`flex-1 py-2 rounded-full text-xs font-bold transition-all ${
            activeTab === 'participants' ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-400'
          }`}
        >
